...
def get_gsheet_client():
  ...
  # แก้ไขวิธีการดึง secrets ให้ถูกต้องตามโครงสร้างของ Streamlit
  creds_dict = {
...
def load_total_cash_balance():
  ...
  try:
    ...
    # (ตรวจสอบชื่อชีตพอร์ตใน Google Sheets ของคุณด้วยว่าเป็น PortfolioData หรือชื่อไทย)
    ...
    if records_portfolio:
      for row in records_portfolio:
        ...
        try:
          ...
          shares = (
              float(str(shares_val).replace(",", ""))
              if shares_val not in [None, ""]
              else 0.0
          )
        ...
        try:
          avg_val = cleaned_row.get(
              "avg_price", cleaned_row.get("ต้นทุนเฉลี่ย", 0.0)
          )
          avg_price = (
              float(str(avg_val).replace(",", ""))
        ...
        with tab_gold:
                ...
                if submitted:
                    ...
                    if gold_type != "เทรดทอง / กองทุนทอง" and weight_input > 0:
                        if gold_type == "ทองคำแท่ง":
                            ...
                            cost_val = init_m_val # ถ้ายึดตามราคาตลาดอ้างอิงตอนซื้อ
                        else:
                            unit_name = "บาททองคำ"
                        ...
                        # บันทึกลง Google Sheets ตาม Header ที่กำหนด
                        try:
                            sheet_gold = get_worksheet_safely(client, 'MyStockData', 'Gold_Portfolio')
                            if sheet_gold is not None:
                                sheet_gold.clear()
            ...
            if 'gold_portfolio' in st.session_state and len(st.session_state['gold_portfolio']) > 0:
                ...
                for idx, row in df_gold.iterrows():
                    ...
                    c_val = row.get("มูลค่าตั้งต้น", val_or_weight)
                    
                    if g_type == "ทองคำแท่ง":
                        market_val = (val_or_weight / 15.244) * ref_gold_bar
                        cost_val = market_val # หรือใช้ค่าตั้งต้นเดิม
                    elif g_type == "ทองรูปพรรณ":
                        market_val = val_or_weight * ref_gold_jewelry
                        cost_val = market_val
                    else:
                        cost_val = c_val
                        market_val = m_val if m_val > 0 else c_val
        ...
        with tab_stock:
                ...
                with tab_portfolio:
                    ...
                    with col_p1:
                        ...
                        if not df_p.empty and 'มูลค่าตลาด' in df_p.columns and 'หุ้น' in df_p.columns:
                            ...
                            st.info("📊 ยังไม่มีข้อมูลในพอร์ตหุ้นสำหรับแสดงกราฟสัดส่วน")
